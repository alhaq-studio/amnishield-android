package com.alhaq.amnishield.ui.fragments.usage
...
import com.alhaq.amnishield.databinding.FragmentAppUsageBreakdownBinding
...
class AppUsageBreakdown(private val stat: AllAppsUsageFragment.Stat) : Fragment() {
    ...
    private fun openCreateRuleForThisApp() {
        val intent = android.content.Intent(requireContext(), com.alhaq.amnishield.ui.activity.FragmentActivity::class.java).apply {
            putExtra("fragment", com.alhaq.amnishield.ui.fragments.BlocksManagerFragment.FRAGMENT_ID)
    ...
    private fun sendAppBlockerRefresh() {
        requireContext().sendBroadcast(
            android.content.Intent(AmniShieldAccessibilityService.INTENT_ACTION_REFRESH_APP_BLOCKER)
    ...
    private fun setupChartUI(
        ...
        val primaryColor = MaterialColors.getColor(
            requireContext(), R.attr.colorPrimary, ContextCompat.getColor(
                requireContext(),
                com.alhaq.amnishield.R.color.text_color
            )
        )
        lineDataSet.apply {
            color = primaryColor
    ...
    private fun showSetLaunchLimitDialog() {
        try {
            ...
            val dialog = com.alhaq.amnishield.ui.dialogs.SetLaunchLimitDialog(
                packageName = stat.packageName,
                appName = appName,
                onSave = { rule ->
                    if (rule != null) {
