...
import com.alhaq.amnishield.ui.screens.BlocksManagerScreen
import com.alhaq.amnishield.ui.screens.CreateRuleScreen
import com.alhaq.amnishield.ui.screens.CreateKeywordBlockerRuleScreen
import com.alhaq.amnishield.ui.screens.CreateWebsiteBlockerRuleScreen
import com.alhaq.amnishield.ui.screens.CreateReelsBlockerRuleScreen
import com.alhaq.amnishield.ui.screens.CreateFocusModeRuleScreen
...
class BlocksManagerFragment : Fragment() {
    ...
    override fun onDestroyView() {
        super.onDestroyView()
        (activity as? com.alhaq.amnishield.ui.activity.MainActivity)?.setBottomNavVisible(true)
    }
    ...
    override fun onCreateView(
        ...
        return ComposeView(requireContext()).apply {
            setContent {
                ...
                var editingRule by remember { mutableStateOf<com.alhaq.amnishield.ui.state.ScheduleRule?>(null) }
                ...
                val checkPinAndProceed: (() -> Unit) -> Unit = { action ->
                    ...
                    val needsPin = pinEnabled && pinCode.isNotEmpty() && !com.alhaq.amnishield.AmniShield.isBypassSessionActive()
                    if (needsPin) {
                ...
                LaunchedEffect(currentScreen) {
                    ...
                    val mainActivity = activity as? com.alhaq.amnishield.ui.activity.MainActivity
                ...
                AmniShieldTheme(appTheme = activeTheme) {
                    if (pendingPinProtectedAction != null) {
                        ...
                        com.alhaq.amnishield.ui.components.PinPromptDialog(
                            correctPin = pinCode,
                            ...
                            onPinSuccess = {
                                com.alhaq.amnishield.AmniShield.unlockBypassSession()
                    ...
                    when (currentScreen) {
                        "manage" -> {
                            BlocksManagerScreen(
                                ...
                                onUpdateRuleAuth = { rule, newAuthType, newHash, newSalt ->
                                    updateRuleAuth(rule, newAuthType, newHash, newSalt)
                                },
                                onNavigateToPremium = { openPremiumScreen() }
                            )
    ...
    private fun openPremiumScreen() {
        try {
            ...
            android.util.Log.e("BlocksManagerFragment", "Failed to open premium screen", e)
        }
    }

    private fun safeOnBack() {
        if (!parentFragmentManager.popBackStackImmediate()) {
            if (activity is com.alhaq.amnishield.ui.activity.FragmentActivity) {
    ...
    private fun loadSchedulesAndLimits() {
        ...
        try {
            ...
            allGroupIds.forEach { groupId ->
                ...
                if (associatedApps.isNotEmpty()) {
                    ...
                    val primaryAppRule = associatedApps.firstOrNull()
                    val ruleAuthType = primaryAppRule?.authType ?: associatedLaunchLimit?.authType ?: com.alhaq.amnishield.security.AuthType.NONE
                    val rulePasswordHash = primaryAppRule?.rulePasswordHash ?: associatedLaunchLimit?.rulePasswordHash
                    val rulePasswordSalt = primaryAppRule?.rulePasswordSalt ?: associatedLaunchLimit?.rulePasswordSalt
        ...
        try {
            ...
            if (!hasKeywordRule && (blockedKeywords.isNotEmpty() || isKwAdultPack || isKwEnabled)) {
                ...
                val kwExisting = appSchedules.firstOrNull { it.groupId == kwRuleId || it.packageName == "keyword_blocker" }
                rulesList.add(
                    com.alhaq.amnishield.ui.state.ScheduleRule(
                        id = kwRuleId,
                        ...
                        authType = kwExisting?.authType ?: com.alhaq.amnishield.security.AuthType.NONE,
                        rulePasswordHash = kwExisting?.rulePasswordHash,
                        rulePasswordSalt = kwExisting?.rulePasswordSalt
                    )
                )
            ...
            if (!hasWebsiteRule && (blockedWebsites.isNotEmpty() || isWebEnabled)) {
                ...
                val webExisting = appSchedules.firstOrNull { it.groupId == webRuleId || it.packageName == "website_blocker" }
                rulesList.add(
                    com.alhaq.amnishield.ui.state.ScheduleRule(
                        id = webRuleId,
                        ...
                        isAlwaysBlockEnabled = true,
                        authType = webExisting?.authType ?: com.alhaq.amnishield.security.AuthType.NONE,
                        rulePasswordHash = webExisting?.rulePasswordHash,
                        rulePasswordSalt = webExisting?.rulePasswordSalt
                    )
                )
            ...
            if (!hasReelsRule && (isReelsEnabled || reelsLimit > 0)) {
                ...
                val reelsExisting = appSchedules.firstOrNull { it.groupId == reelsRuleId || it.packageName == "reel_blocker" }
                rulesList.add(
                    com.alhaq.amnishield.ui.state.ScheduleRule(
                        id = reelsRuleId,
                        ...
                        isAlwaysBlockEnabled = true,
                        authType = reelsExisting?.authType ?: com.alhaq.amnishield.security.AuthType.NONE,
                        rulePasswordHash = reelsExisting?.rulePasswordHash,
                        rulePasswordSalt = reelsExisting?.rulePasswordSalt
                    )
                )
            ...
            if (!hasFocusRule && (focusData.isTurnedOn || focusData.selectedApps.isNotEmpty())) {
                ...
                val focusExisting = appSchedules.firstOrNull { it.groupId == focusRuleId || it.packageName == "FOCUS_MODE" || it.packageName == "focus_mode" }
                rulesList.add(
                    com.alhaq.amnishield.ui.state.ScheduleRule(
                        id = focusRuleId,
                        ...
                        isAlwaysBlockEnabled = true,
                        authType = focusExisting?.authType ?: com.alhaq.amnishield.security.AuthType.NONE,
                        rulePasswordHash = focusExisting?.rulePasswordHash,
                        rulePasswordSalt = focusExisting?.rulePasswordSalt
                    )
                )
            ...
            if (allBlockedApps.isNotEmpty()) {
                ...
                val directExisting = appSchedules.firstOrNull { it.groupId == directAppRuleId || allBlockedApps.contains(it.packageName) }
                rulesList.add(
                    com.alhaq.amnishield.ui.state.ScheduleRule(
                        id = directAppRuleId,
                        name = "App Blocker",
                        ...
                        isAlwaysBlockEnabled = true,
                        authType = directExisting?.authType ?: com.alhaq.amnishield.security.AuthType.NONE,
                        rulePasswordHash = directExisting?.rulePasswordHash,
                        rulePasswordSalt = directExisting?.rulePasswordSalt
                    )
                )
                if (isAppBlockerOn) {
            ...
            launchLimits.filter { !groupAppPkgs.contains(it.packageName) }.forEach { limit ->
                ...
                rulesList.add(
                    com.alhaq.amnishield.ui.state.ScheduleRule(
                        ...
                        authType = limit.authType,
                        rulePasswordHash = limit.rulePasswordHash,
                        rulePasswordSalt = limit.rulePasswordSalt
                    )
                )
            }
        } catch (e: Throwable) {
        ...
        val isAdvanced = true
        val isPremium = try {
            com.alhaq.amnishield.premium.PremiumManager.getInstance(requireContext().applicationContext).isPremium()
        } catch (e: Throwable) {
            false
        }
    ...
    fun updateRuleAuth(
        rule: com.alhaq.amnishield.ui.state.ScheduleRule,
        newAuthType: com.alhaq.amnishield.security.AuthType,
        newHash: String?,
        newSalt: String?
    ) {
        ...
        appRules.forEachIndexed { idx, item ->
            if (item.groupId == rule.id || item.id == rule.id || (rule.selectedApps.isNotEmpty() && rule.selectedApps.contains(item.packageName))) {
                appRules[idx] = item.copy(
                    authType = newAuthType,
                    rulePasswordHash = newHash,
                    rulePasswordSalt = newSalt
                )
                modified = true
            }
        }
        ...
        if (rule.id.startsWith("limit::")) {
            val pkg = rule.id.removePrefix("limit::")
            val limitRule = savedPreferencesLoader.getAppLaunchLimitRule(pkg)
            if (limitRule != null) {
                savedPreferencesLoader.addAppLaunchLimitRule(
                    limitRule.copy(
                        authType = newAuthType,
                        rulePasswordHash = newHash,
                        rulePasswordSalt = newSalt
        ...
        sendRefreshRequest()
        loadSchedulesAndLimits()
        val msg = if (newAuthType == com.alhaq.amnishield.security.AuthType.NONE) "Rule protection removed" else "Rule lock updated"
        Toast.makeText(requireContext(), msg, Toast.LENGTH_SHORT).show()
    }

    private fun saveScheduleRule(rule: com.alhaq.amnishield.ui.state.ScheduleRule) {
    ...
    private fun sendRefreshRequest() {
        ...
        val appIntent = Intent(AmniShieldAccessibilityService.INTENT_ACTION_REFRESH_APP_BLOCKER)
        ...
        val focusIntent = Intent(AmniShieldAccessibilityService.INTENT_ACTION_REFRESH_FOCUS_MODE)
        context.sendBroadcast(focusIntent.setPackage(context.packageName))

        val reelIntent = Intent(AmniShieldAccessibilityService.INTENT_ACTION_REFRESH_REEL_BLOCKER)
        context.sendBroadcast(reelIntent.setPackage(context.packageName))

        val keywordIntent = Intent(AmniShieldAccessibilityService.INTENT_ACTION_REFRESH_BLOCKED_KEYWORD_LIST)
        context.sendBroadcast(keywordIntent.setPackage(context.packageName))

        val viewIntent = Intent(AmniShieldAccessibilityService.INTENT_ACTION_REFRESH_VIEW_BLOCKER)
