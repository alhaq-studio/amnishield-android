 ...
 * 
 * Execution Context:
 * Runs synchronously within [AmniShieldAccessibilityService.onAccessibilityEvent].
...
import com.alhaq.amnishield.ui.activity.AntiUninstallPasswordActivity
...
class AntiUninstallDetector(private val context: Context) {
    ...
    fun inspect(event: AccessibilityEvent?, rootNode: AccessibilityNodeInfo?): Boolean {
        ...
        if (rootPkg.equals(myPackageName, ignoreCase = true) ||
            rootPkg.equals("com.alhaq.amnishield", ignoreCase = true) ||
        ...
        // Check if this screen refers to AmniShield / DeenShield
        val hasAmniShieldMention = matchesAppIdentity(screenData.allTextCombined)

        // 1. Google Play Store Details Page & Uninstall Flow
        if (rootPkg.equals(PLAY_STORE_PACKAGE, ignoreCase = true)) {
            if (hasAmniShieldMention && (screenData.hasUninstallAction || screenData.allTextCombined.contains("uninstall"))) {
        ...
        if (isPackageInstallerPackage(rootPkg)) {
            if (hasAmniShieldMention && (screenData.hasUninstallAction || screenData.allTextCombined.contains("uninstall") || screenData.allTextCombined.contains("remove") || screenData.allTextCombined.contains("delete"))) {
                return executeAntiUninstallBlock()
            }
            return false
        }
        ...
        if (isSettingsPackage(rootPkg)) {
            if (hasAmniShieldMention) {
                ...
                // Vector B: Accessibility Service Screen for AmniShield
    ...
    private fun scanScreenNodes(root: AccessibilityNodeInfo): ScreenData {
        ...
        while (stack.isNotEmpty()) {
            ...
            if (nodeText.contains("use service") || nodeText.contains("use amnishield") ||
                nodeText.contains("use amnishield") || nodeText.contains("shortcut") ||
    ...
    private fun checkIfDateNotReached(): Boolean {
        ...
        if (targetUnlockMillis > 0L) {
            ...
            if (now >= targetUnlockMillis) {
                ...
                val refreshIntent = Intent(AmniShieldAccessibilityService.INTENT_ACTION_REFRESH_ANTI_UNINSTALL)
